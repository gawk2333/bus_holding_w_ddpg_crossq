...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, mask: Optional[Tensor] = None) -> Tuple[Tensor, Tensor]:
        ...
        score = torch.bmm(query.permute(1,0,2), key.permute(1,2,0))/self.sqrt_dim
        if mask is not None:
            score.masked_fill_(mask.view(score.size())==1, -float('Inf'))

        attn = F.softmax(score, -1)
        context = torch.bmm(attn, value.transpose(0,1))
        return context, attn
...
class MultiHeadAttention(nn.Module):
    ...
    Returns: output, attn
        ...
        - **attn** (batch * num_heads, v_len): tensor containing the attention (alignment) from the encoder outputs.
    """
    def __init__(self, d_model: int = 512, d_proj: int = 200, num_heads: int = 4):
        super(MultiHeadAttention, self).__init__()

        assert d_model % num_heads == 0, "d_model % num_heads should be zero."

        self.d_head = int(d_proj / num_heads)
        self.num_heads = num_heads
    ...
    def forward(
        ...
        value = self.key_proj(value).view(batch_size, -1, self.num_heads, self.d_head)  # BxV_LENxNxD
        ...
        context = context.permute(1, 2, 0, 3).contiguous().view(batch_size, self.num_heads * self.d_head)  # BxTxND
